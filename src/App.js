...
const initialState = {
  ...
  taskImage: imagesList[0].imageUrl,
  isGameOver: false,
}
...
class App extends Component {
  ...
  componentDidMount() {
    this.intervalId = setInterval(this.countDownStarts, 1000)
  }
  ...
  isMathThumbnailItem = id => {
    const {taskImage} = this.state
    const getTaskObject = imagesList.find(each => each.imageUrl === taskImage)
    const isMatched = id === getTaskObject.id
  ...
  getImageList = () => {
    ...
    const updatedImageList = imagesList.filter(
  ...
  getStartGame = () => {
    const {taskImage, category} = this.state
    const filteredImageList = this.getImageList()

    return (
      <div className="game-container">
        <img className="task-img" src={taskImage} alt="match" />
          ...
          <ul className="tab-list-container">
            {tabsList.map(each => (
              <TabItems
                eachTabDetails={each}
                key={each.tabId}
                clickedTabId={this.clickedTabId}
                isActiveTabId={category === each.tabId}
              />
            ))}
          </ul>
        </div>
        <div className="thumbnail-container">
          <ul className="thumbnail-list-container">
            {filteredImageList.map(each => (
              <ThumbnailItems
                eachThumbnailDetails={each}
                key={each.id}
                isMathThumbnailItem={this.isMathThumbnailItem}
              />
            ))}
          </ul>
        </div>
  ...
  getGameOveDashboard = () => {
    ...
    return (
      <div className="game-over-dashboard">
        <img
    ...
    return (
      <div className="bg-container">
        ...
        <div className="inner-container">
          {isGameOver ? this.getGameOveDashboard() : this.getStartGame()}
        </div>
      </div>
    )
  }
